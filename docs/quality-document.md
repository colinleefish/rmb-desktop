...
| `internal/hook`, `internal/setup` (agent integrations) | A | B | B | A | B | cursor/cc/codex/opencode/pi/workbuddy/zcode; ZCode payload shape now verified against the installed client bundle (#61 fix, 2026-08-29): Stop carries no user prompt, so UserPromptSubmit capture hook + sidecar pairing — F07's "inferred from docs" flag resolved |
