...
- Event time semantics: events list/filter by WHEN things happened (occurred_at, from the date in the slug/body) — a June decision backfilled in August still sorts under June. `--by=updated` restores write-time view (updated_at).
