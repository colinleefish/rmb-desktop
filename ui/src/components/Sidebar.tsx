...
import { MEMORY_CATEGORIES } from "../lib/memoryCategories";
...
function SidebarNavChildren({ items }: { items: NavChild[] }) {
      ...
      <div>
        ...
        >
          ...
          {item.countKey && counts && (
            <span className="rounded bg-rmb-light px-1.5 py-0.5 text-[10px] tabular-nums text-rmb-gray">
              {counts[item.countKey]}
            </span>
          )}
        </div>
        <SidebarNavChildren items={item.children} />
      </div>
    );
  }
...
export function Sidebar() {
  ...
  const groups: { label: string; hint?: string; items: NavItem[] }[] = [
    {
      label: t.nav.home,
      items: [
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3">
        {groups.map((group) => (
          <div key={group.label} className="mb-4">
            ...
            {group.hint && (
              <p className="px-2 pb-2 text-[11px] leading-snug text-rmb-gray/60">
                {group.hint}
              </p>
            )}
            <ul className="space-y-0.5">
              {group.items.map((item) => (
                <li key={item.to}>
                  <SidebarNavItem item={item} counts={counts} soonLabel={t.nav.soon} />
                </li>
              ))}
            </ul>
          </div>
