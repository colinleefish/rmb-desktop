...
cd webui
npm ci            # first time
npm run dev:mock  # mock mode: full app on in-memory data, no daemon needed
npm run dev       # real mode: vite proxies /api + /healthz to the running rmbd
```

- **`npm run dev:mock`** (`VITE_MOCK=true`) serves every `/api` call from the in-memory mock
...
- **`npm run dev`** proxies to the daemon (default `127.0.0.1:19019`, override with
  `RMB_API_TARGET=http://… npm run dev`).
...
- `npx tsc -b && npx oxlint src` — static gate (F10 L1).
- `npx esbuild src/lib/mock/selftest.ts --bundle --format=esm --outfile=.mock-selftest.mjs && node .mock-selftest.mjs`
